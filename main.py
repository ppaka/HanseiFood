...
async def getInfoNextNextday(ctx: commands.context.Context):
    ...
    nowDate = datetime.datetime.today() + datetime.timedelta(days=2)
    nowStr = str(nowDate)  # 다음날
    year = nowStr[:4]
    month = nowStr[5:7]
    date = nowStr[8:10]
    ...
    if data == '':
        ...
        embed = discord.Embed(
            title='급식 정보', description='내일 모레 급식이야!', color=0xFAEBD7)
        embed.add_field(name='🍽', value=f'{data}', inline=False)
        embed.set_footer(text=f'{month}월 {date}일 / paka#8285')
        await ctx.send(embed=embed)
...
async def getInfoNextNextNextday(ctx: commands.context.Context):
    ...
    if data == '':
        ...
        embed = discord.Embed(
            title='급식 정보', description='내일 모레 모레 급식이야!', color=0xFAEBD7)
        embed.add_field(name='🍽', value=f'{data}', inline=False)
        embed.set_footer(text=f'{month}월 {date}일 / paka#8285')
        await ctx.send(embed=embed)
...
async def getInfoNextNextday(ctx: commands.context.Context):
    ...
    if data == '':
        ...
        embed = discord.Embed(
            title='급식 정보', description='내일 모레 모레 모레 급식이야!', color=0xFAEBD7)
        embed.add_field(name='🍽', value=f'{data}', inline=False)
        embed.set_footer(text=f'{month}월 {date}일 / paka#8285')
        await ctx.send(embed=embed)
