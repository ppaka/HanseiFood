...
class getLunchData(commands.Cog):
    ...
    async def getDayLunch(self, interaction: Interaction, yyyymmdd: int) -> None:
        ...
        if schoolData == None:
            ...
            embed.add_field(name="사용하시기 전에...", value="『/급식학교설정』 명령어로 설정해주세요!")
        ...
        try:
            ...
            embed.add_field(
                name=f"{ymd} 급식 데이터를 조회하는 도중 오류가 발생했습니다.",
        ...
        if (
            school_menu["mealServiceDietInfo"][0]["head"][1]["RESULT"]["CODE"]
            != "INFO-000"
        ):
            embed = discord.Embed(title="오류 발생!", description="", color=0xFFA500)
            embed.add_field(
        ...
        splitted_data = school_menu["mealServiceDietInfo"][1]["row"][0][
            "DDISH_NM"
        ].split("<br/>")
        data = ""
...
async def setup(bot: commands.Bot) -> None:
    await bot.add_cog(getLunchData(bot))
